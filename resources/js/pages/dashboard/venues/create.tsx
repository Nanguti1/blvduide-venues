...
export default function DashboardVenueCreate({ categories, features, countries, subscription }: any) {
    return (
        <>
            ...
            <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
                <div className="mb-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                    <h1 className="text-3xl font-semibold text-slate-900 dark:text-white">Add New Venue</h1>
                    <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Create your listing and submit it for admin review once you're ready.</p>
                </div>
                <div className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
                    <div className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                        <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-500">Listing details</p>
                    ...
                    <aside className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                        <div>
                            ...
                            <p className="mt-2 font-semibold text-slate-900 dark:text-slate-100">{subscription?.package?.name ?? 'No active subscription'}</p>
                        </div>
                        <div>
                            <p className="text-sm text-slate-500">Allowance</p>
                            <p className="mt-2 text-slate-900 dark:text-slate-100">{subscription?.package?.max_listings ?? 0} listings</p>
                        </div>
                    </aside>
                </div>
            </div>
